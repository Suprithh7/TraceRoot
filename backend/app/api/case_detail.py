...
from app.services import risk_scoring, graph_builder, gemma_orchestrator, recommendation, report_generator

router = APIRouter(prefix="/cases/{case_id}", tags=["case-detail"])


async def _load_case(case_id: str, user: dict) -> dict:
...
async def copilot(case_id: str, req: CopilotRequest, user: dict = Depends(current_user)):
    case = await _load_case(case_id, user)
    txs = await transactions_coll().find({"case_id": case_id}, {"_id": 0}).to_list(10000)
    graph = graph_builder.build_graph(txs)
    factors = risk_scoring.score(txs)
    total = risk_scoring.total_points(factors)
    ...
    result = await gemma_orchestrator.generate(req.kind, ctx, req.language)
    # cache
...
async def generate_report(case_id: str, user: dict = Depends(current_user)):
    ...
    # Use cached summary if present, else fall back to a deterministic recap.
    ...
    ctx = {
        "case": {**case, "risk_score": total, "risk": bucket, "tx_count": len(txs)},
    ...
    pdf_bytes = report_generator.build_pdf(ctx)
    return Response(
        content=pdf_bytes,
